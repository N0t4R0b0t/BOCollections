...
| `CORS_ALLOWED_ORIGINS` | `http://localhost:5173` (`http://localhost` on the LXC install) | **On the LXC, almost always needs changing** to match the origin(s) you actually browse from (protocol+host+port, comma-separated, no trailing slash). A mismatch fails silently as a 403 on every write request — see [Configuration](./docs/deployment.md#configuration) |
